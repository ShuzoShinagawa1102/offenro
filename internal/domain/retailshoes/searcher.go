...
func (s *Searcher) Search(
	...
	response, err := client.SearchRetailShoesOffersWithResponse(
		ctx,
		shoesCondition.Request,
	)
	if err != nil {
	...
	for _, merchantOffer := range response.JSON200.Offers {
		...
		offers = append(offers, &Offer{Value: modelapi.RetailShoesOffer{
			...
			Product: merchantOffer.Product, Quantity: merchantOffer.Quantity,
		}})
	}
...
func (s *Searcher) Revalidate(
	...
	response, err := client.RevalidateRetailShoesOfferWithResponse(ctx, modelapi.RetailShoesRevalidateRequest{
	...
	agentOffer := modelapi.RetailShoesOffer{
		Amount: merchantOffer.Amount, Currency: merchantOffer.Currency, Domain: Domain.String(),
		MerchantId: string(capability.Merchant.ID), OfferId: offerID,
		Product: merchantOffer.Product, Quantity: merchantOffer.Quantity,
	}
	if merchantOffer.OfferExpiresAt != nil {

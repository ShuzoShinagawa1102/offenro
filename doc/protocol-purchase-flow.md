...
```text
Search → Cart → Checkout → Purchase CREATED → Merchant Fulfillment → Purchase CONFIRMED
```
...
- `Merchant Fulfillment`はPurchase ItemごとにMerchant側の注文・予約を作成する。全件成立時だけPurchaseを`CONFIRMED`にする。
...
`internal/core/cart`は入力検証、Cartの状態遷移、Checkout時のPurchase生成を担当する。`internal/core/fulfillment`は冪等性、Merchant結果の追跡、Purchase状態の集約を担当する。いずれもDB技術へ依存しない。
